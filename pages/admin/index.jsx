  ...
  return (
    <>
      ...
      <div onKeyPress={({key}) => {if (key == "Enter") authenticate()}} className="mx-auto mt-5 max-w-[350px] bg-white rounded-lg border flex flex-col p-4 gap-1">
        ...
        <input autoFocus onChange={({target}) => setUsername(target.value)} className={"p-2 border rounded mb-2 hover:bg-neutral-100 " + (error ? 'border-red-500 bg-red-50' : null)} type="text" placeholder="Administrator" />
        <p className="text-sm text-neutral-500 ml-1">Password</p>
        <input onChange={({target}) => setPassword(target.value)} className={"p-2 border rounded mb-2 hover:bg-neutral-100 " + (error ? 'border-red-500 bg-red-50' : null)} type="password" placeholder="••••••••" />
